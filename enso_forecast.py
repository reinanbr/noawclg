...
def _compute_oni(
    sst: pd.Series, clim_s: int = CLIM_START, clim_e: int = CLIM_END
) -> pd.Series:
...
def _godas_t200_nino34(year_start: int, year_end: int) -> pd.Series:
    ...
    s = pd.Series(
        mean.values, index=pd.DatetimeIndex(mean["time"].values), name="T200_Nino34"
    )
    return s
...
def _historical_analogs(
    ...
    for yr in range(1950, current_yr):
        ...
        records.append(
            dict(year=yr, dist=dist, mean=yr_mean, trend=yr_trend, end_oni=end_oni)
        )
...
def assess_probability(
    ...
    if oni_current >= 2.0:
        scores["oni"] = 1.00
    elif oni_current >= 1.5:
        scores["oni"] = 0.92
    elif oni_current >= 0.5:
        scores["oni"] = 0.75
    elif oni_current >= 0.0:
        scores["oni"] = 0.45
    elif oni_current >= -0.5:
        scores["oni"] = 0.20
    else:
    ...
    if oni_trend_3m >= 0.4:
        scores["trend"] = 0.92
    elif oni_trend_3m >= 0.15:
        scores["trend"] = 0.72
    elif oni_trend_3m >= 0.0:
        scores["trend"] = 0.50
    elif oni_trend_3m >= -0.2:
        scores["trend"] = 0.30
    else:
        scores["trend"] = 0.10

    # 3. Anomalia T200 Niño 3.4 (peso 0.25) — precursor subsuperficial
    if t200_anom_current >= 1.5:
        scores["t200"] = 0.95
    elif t200_anom_current >= 0.8:
        scores["t200"] = 0.82
    elif t200_anom_current >= 0.3:
        scores["t200"] = 0.65
    elif t200_anom_current >= 0.0:
        scores["t200"] = 0.48
    elif t200_anom_current >= -0.5:
        scores["t200"] = 0.25
    else:
        scores["t200"] = 0.10

    # 4. SSH no Pacífico Leste (proxy de onda de Kelvin) (peso 0.15)
    if ssh_ep_current >= 0.10:
        scores["ssh"] = 0.88
    elif ssh_ep_current >= 0.04:
        scores["ssh"] = 0.68
    elif ssh_ep_current >= 0.0:
        scores["ssh"] = 0.50
    elif ssh_ep_current >= -0.05:
        scores["ssh"] = 0.32
    else:
        scores["ssh"] = 0.12
    ...
    if prob >= 0.75:
        ...
        category = "Moderada-Alta"
    elif prob >= 0.40:
        category = "Moderada"
    elif prob >= 0.25:
        category = "Baixa-Moderada"
    else:
        category = "Baixa"
...
def _plot_oni_series(
    ax: plt.Axes, oni: pd.Series, title: str = "ONI 2010–2026"
) -> None:
    t, v = oni.index, oni.values
    ax.fill_between(
        t, v, 0, where=v >= 0.5, color="#d73027", alpha=0.35, label="El Niño (≥+0.5°C)"
    )
    ax.fill_between(
        t, v, 0, where=v <= -0.5, color="#4575b4", alpha=0.35, label="La Niña (≤-0.5°C)"
    )
    ax.fill_between(
        t,
        v,
    ...
    for yr in NINO_STRONG_YEARS:
        ...
        if len(yr_vals):
            ax.axvline(
                pd.Timestamp(f"{yr}-01-01"), color="#d73027", lw=0.8, ls=":", alpha=0.6
            )
            ax.text(
...
def _plot_t200_series(ax: plt.Axes, t200_anom: pd.Series) -> None:
    ...
    ax.set_title(
        "Temperatura 200 m — Anomalia Niño 3.4", fontsize=10, fontweight="bold"
    )
    ax.grid(axis="y", ls="--", alpha=0.3)
...
def _plot_field_map(
    ...
    pcm = ax.pcolormesh(
        lons, lats, data, cmap=cmap, vmin=vmin, vmax=vmax, shading="auto"
    )
    plt.colorbar(
        pcm, ax=ax, orientation="horizontal", fraction=0.048, pad=0.08, label=label
    )
...
def _plot_assessment(
    ...
    ax.text(
        0.5,
        0.97,
        "Avaliação de El Niño — Próximos 6 Meses",
    ...
    bar_ax.barh([0], [1.0], color="none", height=0.7, edgecolor="#888", linewidth=0.8)
    bar_ax.set_xlim(0, 1)
    ...
    y0 = 0.67
    ax.text(
        0.05, y0, "Indicadores:", transform=ax.transAxes, fontsize=9, fontweight="bold"
    )
...
def run_analysis() -> None:
    if not _HAS_MPL:
        raise ImportError(
            "matplotlib is required to run the analysis. Install it with: pip install matplotlib"
        )
    ...
    oni_trend_3m = float(np.polyfit(range(len(oni_3m_vals)), oni_3m_vals.values, 1)[0])
    ...
    gs = gridspec.GridSpec(
        4, 2, figure=fig, height_ratios=[1.2, 0.9, 1.0, 1.1], hspace=0.42, wspace=0.30
    )
    ...
    if ssh_map is not None:
        month_name = (
            TODAY.strftime("%B/%Y")
            if ssh_month_idx == LATEST_GODAS_MONTHS - 1
    ...
    _plot_assessment(
        ax6, result, oni_last_val, t200_cur, ssh_ep, analog_prob, current_phase
    )
...
def _plot_indicator_bars(ax: plt.Axes, result: dict) -> None:
    ...
    colors = [
        "#d73027" if v > 60 else ("#4575b4" if v < 40 else "#f0a500") for v in vals
    ]
...
def _make_indicators_figure(
    ...
    fig.suptitle("Contexto Histórico ENOS — noawclg", fontsize=12, fontweight="bold")
    ...
    for yr in [1972, 1982, 1997, 2015, 2023]:
        ax.axvline(
            pd.Timestamp(f"{yr}-06-01"), color="#d73027", lw=0.7, ls=":", alpha=0.7
        )
        ax.text(
    ...
    ax2 = axes[1]
    oni_clim = oni_full[
        (oni_full.index.year >= CLIM_START) & (oni_full.index.year <= CLIM_END)
    ]
    ...
    ax2.set_xticklabels(month_labs, fontsize=8)
    ax2.set_ylabel("ONI (°C)", fontsize=9)
    ax2.set_title(
        "Ciclo sazonal do ONI e posição de 2026 na climatologia",
        fontsize=10,
        fontweight="bold",
    )
...
def _print_summary(
    ...
    print()
    print("  ┌─────────────────────────────────────────────┐")
    print("  │  PROBABILIDADE EL NIÑO (6 meses)            │")
...
def _print_enso_context(prob: float) -> None:
    if prob >= 0.75:
        msg = (
            "  O conjunto de indicadores aponta fortemente para El Niño.\n"
        ...
        msg = (
            "  Sinais favoráveis ao El Niño, mas ainda inconclusivos.\n"
        ...
        msg = (
            "  Indicadores mistos — sem sinal claro de El Niño.\n"
        ...
        msg = (
            "  Indicadores apontam para La Niña ou condições neutras.\n"
