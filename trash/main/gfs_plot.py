...
from noawclg.gfs_dataset import GFSDatasetManager  # noqa: E402
...
def _frame_painel(ds_g, ds_br, tidx: int, hour: int) -> plt.Figure:
    ...
    _title(ax, "PRMSL — Ortho", rl)
    _colorbar(fig, cf, ax, "hPa")
    ...
    _title(ax, "Precipitação — Brasil", rl)
    _colorbar(fig, cf, ax, "mm h⁻¹")

    # ── [1,1] Vento Global PlateCarree ────────────────────────────────────────
    ax = fig.add_subplot(gs[1, 1], projection=ccrs.PlateCarree())
    ax.set_global()
    _add_features(ax, 0.35)
    _add_gridlines(ax, ccrs.PlateCarree())
    ...
    _title(ax, "Vento 10m — Global", rl)
    _colorbar(fig, cf, ax, "m s⁻¹")
