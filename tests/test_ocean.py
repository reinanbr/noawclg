...
def _make_godas_ds(variable: str = "pottmp", n_levels: bool = True) -> xr.Dataset:
    ...
    if n_levels:
        ...
        da = xr.DataArray(
            ...
            attrs={"missing_value": np.float32(9.969209968386869e36)},
        )
    else:
        # 2-D field: (time, lat, lon)  — sshg
        raw = np.random.uniform(-0.3, 0.3, (len(t), len(_LAT), len(_LON))).astype(
            np.float32
        )
        da = xr.DataArray(
            raw,
...
def _make_ersst_ds() -> xr.Dataset:
    ...
    sst = xr.DataArray(
        ...
        attrs={"missing_value": np.float32(9.969209968386869e36), "units": "degC"},
    )
...
class TestOpenGodas:
    ...
    def test_region_selection_reduces_lat_lon(self, mock_pottmp):
        region = {"lat_min": -5.0, "lat_max": 5.0, "lon_min": 190.0, "lon_max": 240.0}
